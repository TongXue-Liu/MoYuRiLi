...
import { getJueJinHot } from '@/api/hot.js';
// 剪贴板操作
...
//返回的热点数据
let activities = ref({});
const getJueJinHotHandler = async() => {
    await getJueJinHot().then((res) => {
...
// 暴露刷新方法
const refresh = () => {
    console.log("JueJin Refresh……");
    return getJueJinHotHandler();
}

// onMounted
onMounted(() => {
    getJueJinHotHandler();  //获取热点数据
})
